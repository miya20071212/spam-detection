...
本项目基于约80万条原始短信数据，构建了完整的垃圾短信与诈骗邮件智能检测流程，包括数据清洗、样本平衡、中文分词、特征提取以及多种机器学习与深度学习模型训练。

实验结果表明，传统机器学习模型能够有效完成垃圾信息分类任务，其中 RBF-SVM 模型取得 98.63% 的准确率；深度学习模型进一步提升了分类性能，其中 LSTM 模型以 98.88% 的准确率和 0.9887 的 F1-score 取得最佳实验结果。

此外，项目还完成了 BERT 预训练语言模型及多学习率消融实验。结果显示，BERT 模型在不同学习率下均具有较好的稳定性和泛化能力，但其最佳准确率为 97.87%，未超过 LSTM 模型以及两种 SVM 模型。

综合实验结果可知，在文本长度较短且类别特征较为集中的垃圾短信检测场景中，LSTM 模型表现出最佳的综合性能，验证了所构建检测系统的有效性。
